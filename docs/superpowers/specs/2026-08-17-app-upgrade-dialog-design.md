...
6. **Store launcher** (platform channel — replaces the current `openAppStore` / `openGooglePlayStore` stubs)
   - Android (Kotlin): triggers Google Play's In-App Update API (`com.google.android.play:app-update`, `AppUpdateManager`) with `AppUpdateType.IMMEDIATE` — a full-screen native Play UI that installs the update without the user leaving the app. This requires the plugin to implement `ActivityAware` (Play Core's `startUpdateFlowForResult` needs an `Activity`, not just an application `Context`) and track the currently-attached activity. `androidPackageId` is not passed to Play Core at all — the In-App Update API always operates on the currently-running app's own installation; the id is only used to build the fallback URL. If no `Activity` is attached, Play Core reports the update isn't available or `IMMEDIATE` isn't an allowed type (e.g. sideloaded install, no update indexed yet), or the flow call itself throws, falls back to opening `https://play.google.com/store/apps/details?id=<package>` via `ACTION_VIEW` — the same fallback URL as before.
   - iOS (Swift): presents the App Store product page in-app via StoreKit's `SKStoreProductViewController`, loaded with `SKStoreProductParameterITunesItemIdentifier: appId` and presented from the app's root view controller — the user never leaves the app. If StoreKit fails to load the product (no root view controller found, network error, invalid id, another modal already presented), falls back to opening `https://apps.apple.com/app/id<appId>` via `UIApplication.shared.open(...)` as a last resort so the user isn't left stuck.
   - `androidPackageId` defaults to the host app's own package name (read on-device) when not passed by the host app, and is used only for the web fallback URL; `iosAppId` has no on-device fallback and must be passed by the host app or the store won't open.
